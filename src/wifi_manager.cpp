...
// Optional: ArduinoJson can be used elsewhere; here we build JSON manually.
...
static unsigned long connectBackoff = 5000; // 5s initial
static const unsigned long MAX_BACKOFF = 5 * 60 * 1000UL; // 5 min

// pointer to AsyncWebServer (if provided by caller)
static AsyncWebServer *gServer = nullptr;

// ------------------------------------------------------------------
// Helper: create QR image URL (Google Charts quick method)
...
// ------------------------------------------------------------------
// Setup captive DNS server (redirect all queries to AP IP)
...
// ------------------------------------------------------------------
// Helper: register basic captive endpoints on provided AsyncWebServer
static void registerCaptiveEndpoints(AsyncWebServer *server) {
  if (!server) return;

  // root page for captive portal (simple)
  ...
  server->on("/wifi_set", HTTP_POST, [](AsyncWebServerRequest *req){
    ...
    // Save to config and attempt connect
  ...
  // Scan endpoint (returns simple HTML list)
  ...
  // Simple API to get AP IP and QR link
...
// ------------------------------------------------------------------
// Public: start AP and optionally attach captive portal server
void startAP() {
  // Use configured AP SSID/pass if present
  ...
  // Start captive DNS if server is present
  if (gServer) startCaptiveDNS();
}

// Public: start STA (attempt connection)
...
// Return the IP (STA if connected else AP)
String wifiIP() {
  if (WiFi.status() == WL_CONNECTED) return WiFi.localIP().toString();
  return apIP.toString();
}

// Return JSON array of scanned networks
...
// Captive redirect helper (call from your web handler to enforce redirect)
void captiveRedirect(AsyncWebServerRequest *request) {
  ...
  // If request host is not the AP IP, redirect
...
// Periodic handler: manage reconnect/backoff and DNS processing
void wifiHandlePeriodic() {
  // Poll DNS server if running
  if (captiveEnabled) dnsServer.processNextRequest();

  // Manage STA reconnect/backoff
  if (config.sta_ssid.length() > 0) {
    if (WiFi.status() != WL_CONNECTED) {
      ...
      // If connected, ensure captive DNS stopped
      if (captiveEnabled) {
        stopCaptiveDNS();
      }
    }
...
// ------------------------------------------------------------------
// Main entrypoint: start wifi and optionally register captive portal endpoints
void wifiStart(AsyncWebServer* server) {
  gServer = server;

  // Ensure LittleFS mounted for any captive files (QR or form)
  ...
  // If saved STA creds exist, try to connect
  if (config.sta_ssid.length() > 0) {
    Serial.printf("[WIFI] Saved STA found, attempting connect to '%s'\n", config.sta_ssid.c_str());
    WiFi.mode(WIFI_MODE_APSTA); // AP + STA
    WiFi.begin(config.sta_ssid.c_str(), config.sta_pass.c_str());
    lastConnectAttempt = millis();
  }

  // If server provided, register captive portal endpoints
  if (server) {
    registerCaptiveEndpoints(server);
  }
}
